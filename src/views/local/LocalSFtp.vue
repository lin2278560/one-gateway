<template>
    <div class="container-fluid">
        <div class="row box margin-x clear-margin padding-x">
            <div class="modal-header " style="border-bottom: 1px solid #a9a9a9;">
                <blockquote style="padding: 0; margin: 0; border: none;color: #000;">
                    ...
                    <sftpAdd style="float: right"></sftpAdd>
                </blockquote>
            </div>
        ...
        <Drawer title="本地SFTP服务" v-model="modal.show" width="50%">
...
<script>
    ...
    import sftpAdd from "./LocalSFtpAdd";
    export default {
        name: "LocalSFtp",
        components: {
            jk_tabs: JkTabs,sftpAdd
        },
        inject: ["reload"],
        data(){
            return {
                ...
                columns1: [
                    ...
                    {title: 'SFTP目录', key: 'path', align: 'center'},
        ...
        methods: {
            ...
            deleteFtp(index,i) {
                ...
                this.$Modal.confirm({
                    ...
                    loading: true,
                    onOk: () => {
                        this.$https.fetchGet(this.$api.local.delSftp.url, {id: index}).then((resp) => {
            ...
            updateState(index){
                if(this.data3[index]["state"] == "stop"){
                    ...
                    this.api = this.$api.local.startSftp.url;
                }else {
                    this.stateTitle = "禁用服务";
                    this.stateContent = "确定要禁用此服务吗?";
                    this.modal.form.state = "stop";
                    this.api = this.$api.local.stopSftp.url;
                }
            ...
            updateFtp(index) {
                ...
                if(this.data3[index].state == "stop") { this.modal.form.state = "stop" }
            ...
            addPlatForm() {
                this.$refs['info_form'].validate((valid) => {
                    if (!valid)
                        return;
                    this.modal.form.path = "/home/local/sftp/"+this.modal.form.username;
                    this.$https.fetchPost(this.$api.local.updateSftp.url, this.modal.form).then((resp) => {
                        if (resp.data.status == 200) {
                            this.$Notice.success({title: 'SFTP服务修改成功', desc: "正在刷新界面", duration: 2.5});
                            setTimeout(() => {
                                this.reload();
                            }, 250);
                        } else {
                            this.$Notice.error({title: 'SFTP服务修改失败', desc: "错误：" + resp.data.desc, duration: 5});
                        }
