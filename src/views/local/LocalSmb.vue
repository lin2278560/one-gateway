<template>
    <div class="container-fluid">
        <div class="row box margin-x clear-margin padding-x">
            <div class="modal-header " style="border-bottom: 1px solid #a9a9a9;">
                <blockquote style="padding: 0; margin: 0; border: none;color: #000;">
                    ...
                    <smbAdd style="float: right"></smbAdd>
                </blockquote>
            </div>
        ...
        <Drawer title="本地SMB服务" v-model="modal.show" width="50%">
            <Form ref="info_form" :rules="modal.rules" :model="modal.form"
                ...
                <!-- <FormItem label="权限" prop="" style="margin-bottom: 15px;">
                    <Checkbox v-model="modal.form.browseable">读</Checkbox>
                    <Checkbox v-model="modal.form.writable">写</Checkbox>
                </FormItem> -->
                <FormItem>
...
<script>
    ...
    import smbAdd from "./LocalSmbAdd";
    export default {
        name: "LocalSmb",
        components: {
            jk_tabs: JkTabs,smbAdd
        },
        inject: ["reload"],
        data(){
            return {
                    ...
                    form: {
                     ...
                     rules:{
                        username: [
                            ...
                            {  max:15, message: '长度不超过15', trigger: 'blur' },
        ...
        methods: {
            ...
            deleteFtp(index,i) {
                console.log(this.data1[i].state)
                if(this.data3[i].state == "start"){
                ...
                this.$Modal.confirm({
                    ...
                    loading: true,
                    onOk: () => {
                        this.$https.fetchGet(this.$api.local.delSam.url, {id: index}).then((resp) => {
            ...
            updateState(index){
                ...
                this.modal.form.browseable = this.data1[index]["browseable"];
                this.modal.form.writable = this.data1[index]["writable"];
                // if(this.data1[index].browseable == "true") {this.modal.form.browseable = true;} else {this.modal.form.browseable = false;}
                // if(this.data1[index].writable == "true") {this.modal.form.writable = true;} else {this.modal.form.writable = false;}
                ...
                this.$Modal.confirm({
                    ...
                    onOk: () => {
                        this.$https.fetchPost(this.api, this.modal.form).then((resp) => {
                            if (resp.data.status == 200) {
                                // this.$Notice.open({title: '启停成功', desc: "正在刷新界面", duration: 2.5});
                                this.$Notice.success({title: '启停成功', desc: "正在刷新界面", duration: 2.5});
                                setTimeout(() => {
                                    this.$Modal.remove();
                                        this.reload();
                                    }, 250);
            ...
            updateFtp(index) {
                console.log(this.data1[index].state)
                if(this.data3[index].state == "start"){
                    this.$Notice.error({title: '启用中无法修改', duration: 2.5});
                    return
                }
                ...
                if(this.data3[index].state == "stop") { this.modal.form.state = "stop" } else { this.modal.form.state = "start" }
                if(this.data1[index].browseable == "true") {this.modal.form.browseable = true;} else {this.modal.form.browseable = false;}
                if(this.data1[index].writable == "true") {this.modal.form.writable = true;} else {this.modal.form.writable = false;}


                console.log(this.modal.form.browseable);
            },
            ...
            addPlatForm() {
                this.$refs['info_form'].validate((valid) => {
                    if (!valid)
                        return;
                    this.modal.form.path = "/home/"+this.modal.form.username;
                    this.$https.fetchPost(this.$api.local.updateSam.url, this.modal.form).then((resp) => {
                        if (resp.data.status == 200) {
                            this.$Notice.success({title: 'Smb服务修改成功', desc: "正在刷新界面", duration: 2.5});
                            setTimeout(() => {
                                        this.reload();
                                    }, 250);
                        } else{
                            this.$Notice.error({title: 'Smb服务修改失败', desc: "错误：" + resp.data.desc, duration: 5});
                        }
